...
(function(){
		...
		items: [{
			...
			items: [{
				...
				store: new Ext.data.JsonStore({
					url: '/store/project/getProjectList.php',
					remoteSort: true,
					idProperty: 'proj_id',
					root: 'data',
					fields: [
				...
				colModel: new Ext.grid.ColumnModel({
					...
					columns: [
						...
						{header: '사용 여부', dataIndex: 'use_yn', width: 70, align:'center', renderer: function(v){ if(v == 'Y') { return '사용'; } else { return '사용안함'; } } },
				...
				*/
					...
					handler: function(btn){
						...
						Ext.Msg.show({
							...
							fn: function(btn){
								if(btn == 'ok'){
									Ext.Ajax.request({
										url: '/store/project/setLicense.php',
										params: {
											action:	'reset',
					...
					handler: function(btn){
						...
						aIframe.src = 
						"/store/project/downloadLicense.php?id="+license_row.get('id')+"&action=offline_license_download";
					...
					handler: function(btn){
						...
						aIframe.style.display = "none";
						aIframe.src = 
						"/store/project/downloadLicense.php?id="+license_row.get('proj_id')+"&action=current_license_state";
				...
				store: new Ext.data.JsonStore({					
					url: '/store/project/getLicenseList.php',
					remoteSort: true,
					sortInfo: {
						field: 'create_date',
						direction: 'DESC'
	...
	function requestProj(flag, registProjForm_proj_data){
		Ext.Ajax.request({
			url: '/store/project/setProject.php',
			params: {
				action:			flag,
	...
	function viewLicenseInformation(licenseIndex){
		...
		var viewLicenseInformation = new Ext.Window({
			...
			items: [{
				...
				//title: 'Mac Address',
				store: new Ext.data.JsonStore({					
					url: '/store/project/getLicenseMacAddress.php',
					remoteSort: true,
					baseParams: {
